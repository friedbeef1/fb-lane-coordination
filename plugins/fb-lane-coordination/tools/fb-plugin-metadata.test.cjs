...
const versionPattern = /^0\.5\.0-beta\+codex\.\d{14}$/;
...
function validatePluginPackage(root) {
  ...
  assert.match(codexManifest.version, versionPattern, 'Codex manifest must use the 0.5.0-beta UTC build ID');
...
if (!isPackagedCopy) {
  ...
  assert.match(read(repoRoot, 'FAQ.md'), /0\.5\.0-beta/, 'FAQ.md intentionally names the release family');
...
try {
  ...
  fs.writeFileSync(path.join(fixtureRoot, 'README.md'), read(pluginRoot, 'README.md').replace(version, '0.5.0-beta+codex.19990101000000'));
  assert.throws(
