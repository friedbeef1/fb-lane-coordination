...
test('documents the completed bootstrap and v2 review-authoring contract across source and package', () => {
  ...
  for (const relativePath of setupSkills) {
    const source = fs.readFileSync(path.join(repoRoot, relativePath), 'utf8');
    assert.match(source, /bootstrap (?:installs|copies) the canonical seven-page (?:FB harness|\[FB harness\]\([^)]*\))\s*pack/i, `${relativePath} must describe the completed pack install`);
