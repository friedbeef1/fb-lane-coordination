...
*   **BFM Return Loop**: When Product/BFM processes all lane handoffs, every handoff must be marked \`implemented\`, \`already done\`, \`blocked\`, \`out of scope\`, or \`explicitly deferred\`. Return to board, handoffs, source/docs/tests, lane status, and git status before closeout. Before the first claim and after every material result, confirm the active checkpoint and that the next action advances the persistent objective. Name whether the branch/worktree is clean, merged, stale, blocked, or intentionally dirty. If checks touched external services, also name test mode, created records/resources, cleanup evidence, or the pending cleanup gate. Add one loop health flag: \`healthy\`, \`watch\`, \`needs Product review\`, or \`blocked\`; do not numeric-score the loop.
...
- Before the first claim, record one persistent objective, definition of done, and current checkpoint; use a runtime task goal when available, otherwise the board/handoff. Treat shorthand such as \`proceed\` or \`all\` as continuation unless the user names a new task, and freeze scope on an unresolved blocker.
