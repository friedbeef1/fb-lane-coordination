...
*   **FB-Product (PM / Integration User Value)**: Owns final product decisions, task prioritization, scoping, file merges, staging/live deployments, and release gates. Product writes one canonical Goal Alignment block per non-trivial task in \`PROJECT_BOARD.md\` (\`Working Goal\`, \`Definition of Done\`, \`Gate / Review Point\`) and records goal drift as \`Goal changed from X to Y because Z.\` when it changes. Product gives direction and integration; the owning lane claims and executes its own task/files.
...
1. **Claim**: Product scopes the item; the owning lane claims its own task/files on the board. For non-trivial tasks Product sets one canonical Goal Alignment block (\`Working Goal\`, \`Definition of Done\`, \`Gate / Review Point\`) before the work starts. Micro quick tasks can skip this extra ceremony.
...
### Lightweight Goal Alignment
- For non-trivial tasks, FB-Product owns one canonical Goal Alignment block in \`PROJECT_BOARD.md\` with \`Working Goal\`, \`Definition of Done\`, and \`Gate / Review Point\`.
- Add \`Goal Alignment\` to the board detail block with \`Working Goal\`, \`Definition of Done\`, and \`Gate / Review Point\` where practical.
