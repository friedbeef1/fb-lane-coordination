...
*   **Completion Audit Rule**: Product reports implementation, automated checks, and Design visual signoff as separate statuses. Do not call Design work "done", "executed", or "visual QA passed" unless screenshot/viewport evidence is present; otherwise mark Design visual QA/signoff as pending.
...
3. **Audit**: When complete, the thread pushes the branch, moves the board item to \`Staging QA\` using \`node tools/fb-lane.cjs submit\`, and records implementation status, automated checks, and Design visual QA evidence separately.
4. **Merge**: \`FB-Product\` runs verification/release gates, verifies screenshot/viewport evidence for Design signoff when UI changed, merges the branch to main using \`node tools/fb-lane.cjs merge\`, and releases locks.
`;
