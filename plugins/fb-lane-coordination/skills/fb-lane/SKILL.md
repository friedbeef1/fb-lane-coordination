...
For non-trivial lane work, keep one canonical Goal Alignment block in `PROJECT_BOARD.md` with `Working Goal`, `Definition of Done`, and `Gate / Review Point`. Product/BFM owns that block. Do not apply this ceremony to micro quick tasks.
...
6. For non-trivial work, confirm the task detail block has one canonical Goal Alignment block in `PROJECT_BOARD.md` with `Working Goal`, `Definition of Done`, and `Gate / Review Point`. Product/BFM writes or changes that block; worker lanes report missing, unclear, or misaligned goals in handoffs.
