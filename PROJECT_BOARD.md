...
|---|---|---|---|---|---|---|
| TASK-085 | In Progress — recovery candidate | FB-Product / BFM + FB-Tech | Six-workstream intake and erased-receipt recovery | Preserve User/Ready-to-ship selection, rebuild historical content receipts without invented routes, and reconcile an approved changed handoff only from exact current hashes plus one prior disposition | Source/package recovery candidate verified; bounded local reinstall remains the next gate; locks retained | 19/19 intake, 35/35 migration, 39/39 onboarding in root/package plus 86/86 parity; no Push Live, merge, push, publication, or release; [Handoff](docs/handoffs/TASK-085.md); [QA](docs/qa/TASK-085.md) |
...
*   **Status**: In Progress — bounded erased-receipt recovery added after the fresh Unmirror consumer failure; exact candidate preflight remains.
