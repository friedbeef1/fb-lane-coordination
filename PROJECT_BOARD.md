...
|---|---|---|---|---|---|---|
| TASK-091 | Checking | FB-Product / BFM + FB-Discovery | FB `0.10.0-beta`; versioned graph contract and legacy exact-task adapter | Preserve the seven proven sidebar tasks, then introduce one machine-readable graph/state/transition contract with v1 read compatibility and fail-closed authority semantics | `tools/fb-onboarding.cjs`, graph contract/runtime/tests, package manifest/mirrors, TASK-091 records | Graph-driven route: implementation and whole-candidate review passed; final release checkpoint remains; [Handoff](docs/handoffs/TASK-091.md); [Discovery input](docs/handoffs/TASK-FB-GRAPH-LOOPS-20260826.md); [QA](docs/qa/TASK-091.md) |
| TASK-FB-GRAPH-LOOPS-20260826 | Done | FB-Discovery → FB-Product / BFM | Compatibility-first graph recommendation | Identify a versioned contract slice without forcing a monolithic graph migration | None; absorbed into TASK-091 | Product dispositioned the recommendation into TASK-091; [Discovery handoff](docs/handoffs/TASK-FB-GRAPH-LOOPS-20260826.md) |
...
*   **Status**: Checking — identity repair, focused proof, packaging, and one whole-candidate review passed; the final release checkpoint remains.
...
*   **Status**: Done — `0.9.3-beta+codex.20260820032957` is published, installed, enabled, and runtime-verified.
*   **Owner / Thread**: FB-Product / BFM.
...
*   **Status**: Done — PR #67 merged as `823e51b`; exact build `0.9.0-beta+codex.20260817211319` is published, installed, enabled, and runtime-verified.
*   **Owner / Thread**: FB-Product / BFM.
...
*   **Status**: Done — source `ef0d064` is inherited by FB 0.9.0, merged through PR #67 as `823e51b`, and installed in exact enabled build `0.9.0-beta+codex.20260817211319`.
...
*   **Status**: Done — PR #66 passed GitHub readiness and merged reviewed head `d6e8abe` as `d310548`; the configured local marketplace was advanced to that exact merge, and `0.8.1-beta+codex.20260810055302` is installed and enabled with 89/89 artifact parity and a working bundled MCP route.
...
*   **Out of Scope**: Consumer-project source changes, installed-cache edits, a new sidebar skill, blind archive/rename, weakened inventory checks, TASK-082 mutation, merge, marketplace publication, reinstall, or deployment.
*   **Goal Alignment Session**:
    ...
    *   **Gate / Review Point**: Complete — James explicitly supplied **Push Live** for TASK-083; PR #66 merged, the marketplace source was refreshed, and the exact plugin build was reinstalled and verified. No MÉJA deployment was authorized or performed.
...
*   **Locks**: None. TASK-082's release records and the canonical checkout's unrelated local setup edits remain untouched.
*   **Links & Deliverables**: [handoff](docs/handoffs/TASK-083.md) · [QA](docs/qa/TASK-083.md) · MÉJA MEJA-122 consumer acceptance is summarized in TASK-083 QA and remains separate from package durability.
