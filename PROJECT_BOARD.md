...
|---|---|---|---|---|---|---|
| TASK-067 | Staging QA | FB-Product / BFM | Conversation execution authority | Make Product/BFM parent execution, workstream planning, and one-use sidechat exceptions explicit without adding runtime machinery | Canonical/package guardrails, Product/BFM and six workstream skills, focused skill contract, coordination records | [Handoff](docs/handoffs/TASK-067.md); [QA](docs/qa/TASK-067.md); local candidate only |
...
| TASK-053 | Staging QA | FB-Product / BFM + FB-Tech | 95% Readiness | Hardened version 3 completed nine fresh one-pass runs; Vanilla, Broad FB, and Preventive Graph FB each passed 20/20 deliverables plus 8/8 blockers in all three repetitions, demonstrating parity on the fixed fixture | TASK-053 benchmark evidence and records only; no plugin adoption, release, or superiority claim | [Handoff](docs/handoffs/TASK-053.md); [Result](docs/benchmarks/control-loop/readiness95.md); [Independent review](docs/benchmarks/control-loop/readiness95-v3-independent-review.md); [QA](docs/qa/TASK-053.md) |
...
| TASK-050 | Done | FB-Product / BFM + FB-Tech | Agent Control Loop | Add generic rules-first routing, clone-local stage events, baseline comparison, layered gates, and isolated golden-fixture configuration evolution for FB 0.5 | New control-loop runtime/tests, session/eval integration, canonical/package harness and skills, TASK-050 records | [Handoff](docs/handoffs/TASK-050.md); [QA evidence](docs/qa/TASK-050.md); published build `0.5.0-beta+codex.20260728113402` |
...
*   **Status**: Staging QA — focused candidate complete; external actions closed.
...
*   **Out of Scope**: Runtime permission machinery, CLI changes, publication,
    marketplace upgrade, install, merge, deployment, or provider mutation.
*   **Goal Alignment Session**:
    ...
    *   **Definition of Done**: Focused skill contract, 53-mirror parity, doctor,
        syntax, links, and whitespace pass.
    *   **Gate / Review Point**: Merge, publication, marketplace upgrade,
        reinstall, and deployment remain unapproved.
    *   **Approval**: approved
...
*   **Scope**: Give agents a bounded active-only board packet and mechanically
    ...
    a conservative size threshold; keep the active harness overview and FAQ
    aligned with the safety and fallback behavior.
...
*   **Scope**: Compare autonomous Vanilla, broad-context FB, and preventive
    graph FB using the same public facts and equal aggregate authority and
    budgets. Let each system choose its agent topology. Use Features, Bugs,
    Tech, and Design scenario families; report 91% (241/264) and 99% (262/264)
    deliverable milestones and require all 24 intentional blockers to remain
    correctly blocked.
...
*   **Definition of Done**: The prevention policy is frozen before the
    ...
    declared predicate.
*   **Affected Screens / Locks**: TASK-052 runner/tests, frozen benchmark
...
*   **Scope**: Close the frozen modeled result: the candidate used 310,358
    modeled token units against the 298,080 maximum (fail) and 555.375 modeled
    minutes against the 557.3 maximum (pass). Readiness remained 231/288
    (80.2%), missed required controls remained zero, immediate safety response
...
### TASK-050 - Generic Agent Control Loop

*   **Status**: Done
*   **Owner / Thread**: FB-Product / BFM + FB-Tech
*   **Area**: Agent orchestration, evidence, quality, and configuration evolution
*   **Scope**: Add a generic rules-first process/skip router, clone-local flat stage-event ledger, baseline comparison, layered gates, bounded diagnosis, and isolated golden-fixture configuration candidates.
*   **Out of Scope**: Transcript or hidden-reasoning capture, hosted telemetry, mandatory agent-per-stage execution, autonomous canonical config promotion, automatic eval promotion, merge, plugin publication, marketplace upgrade, installation, or deployment.
*   **Affected Screens / Locks**: New control-loop runtime/tests; session/eval/doctor/MCP integration; canonical/package harness, skills, metadata, changelog, and TASK-050 records.
*   **Goal Alignment Session**:
    *   **Objective**: Improve production-agent diagnosis, quality, and learning without weakening FB’s repository-local Product authority or materially slowing ordinary work.
    *   **Key Results**: Clear routes are deterministic; ambiguous routes are explicit; operational events are flat, redacted, and clone-local; transformations compare against a baseline; failures produce bounded evidence; golden candidates cannot self-promote.
    *   **Definition of Done**: Both gated tasks pass focused review, package mirrors agree, the release checkpoint passes once, and the candidate stops at Ready to ship.
