...
function assertProductEvidenceBoundary() {
  ...
  assert.match(skill, /queue preview and approval/i);
  assert.match(skill, /wait for[\s\S]{0,30}okay/i);
  assert.doesNotMatch(skill, /without a routine second approval/i);
...
function assertAlignedSkills() {
  ...
  assert.match(bfm, /do not require those briefs to preexist invocation/i);
  assert.match(bfm, /Wait for[\s\S]{0,30}okay/);
  assert.match(bfm, /queue-preview-and-approval/);
  assert.match(bfm, /Before source\s+changes[\s\S]{0,120}Goal Alignment[\s\S]{0,40}Session to match the reconciled briefs/i);
