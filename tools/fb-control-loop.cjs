...
function collectControlLoopDoctorChecks(repoRoot) {
  try {
    ...
    if (/not a git repository|not inside a git repository|Git common directory/i.test(error.message)) {
      return [{ level: 'warn', label: 'Control-loop event registry', detail: 'Clone-local control-loop events are unavailable until Git is initialized.', fix: 'Initialize Git before recording repository-local control-loop events.' }];
    }
