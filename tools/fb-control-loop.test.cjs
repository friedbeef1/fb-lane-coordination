...
test('doctor treats a non-Git optional consumer as unavailable but malformed Git-backed events as failure', () => {
  const nonGit = fs.mkdtempSync(path.join(os.tmpdir(), 'fb-control-loop-non-git-'));
  const fixture = createRepo();
  try {
    const unavailable = collectControlLoopDoctorChecks(nonGit);
    assert.strictEqual(unavailable[0].level, 'warn');
    assert.match(unavailable[0].detail, /unavailable|Git/i);

    const common = git(fixture.repo, ['rev-parse', '--git-common-dir']);
    const commonPath = path.resolve(fixture.repo, common);
    const events = path.join(commonPath, 'fb-lane', 'events');
    fs.mkdirSync(events, { recursive: true });
    fs.writeFileSync(path.join(events, 'bad-run.jsonl'), '{not-json}\n');
    const malformed = collectControlLoopDoctorChecks(fixture.repo);
    assert.strictEqual(malformed[0].level, 'fail');
  } finally {
