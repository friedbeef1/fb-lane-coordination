...
if (!isPackagedCopy) {
  ...
  assert.match(read(repoRoot, 'FAQ.md'), /0\.3\.0-beta/, 'FAQ.md intentionally names the release family');
...
try {
  ...
  fs.writeFileSync(path.join(fixtureRoot, 'README.md'), read(pluginRoot, 'README.md').replace(version, '0.3.0-beta+codex.19990101000000'));
  assert.throws(
