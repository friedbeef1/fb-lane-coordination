...
test('bootstrap installs ten pages and both templates while preserving project-owned eval records and instructions', () => {
  ...
  try {
    ...
    for (const page of ['README.md', 'start.md', 'workflow.md', 'evidence.md', 'guardrails.md', 'sessions.md', 'evals.md', 'records.md', 'graph.md', 'control-loop.md']) assert.ok(fs.existsSync(path.join(root, 'docs', 'fb', page)), page);
...
test('documented fallback command sequence acquires every bootstrap runtime and canonical asset and runs exactly', () => {
  ...
  try {
    ...
    for (const tool of ['fb-lane.cjs', 'fb-session.cjs', 'fb-eval.cjs', 'fb-efficiency.cjs', 'fb-changelog-closeout.cjs', 'fb-records.cjs', 'fb-project-graph.cjs', 'fb-control-loop.cjs']) fs.copyFileSync(path.join(repoRoot, 'tools', tool), path.join(archiveRoot, 'tools', tool));
    ...
    for (const tool of ['fb-lane.cjs', 'fb-session.cjs', 'fb-eval.cjs', 'fb-efficiency.cjs', 'fb-changelog-closeout.cjs', 'fb-records.cjs', 'fb-project-graph.cjs', 'fb-control-loop.cjs']) assert.strictEqual(fs.readFileSync(path.join(root, 'tools', tool), 'utf8'), fs.readFileSync(path.join(repoRoot, 'tools', tool), 'utf8'), tool);
