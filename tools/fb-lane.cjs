...
3. **Audit**: When complete, the thread pushes the branch, moves the board item to \`Staging QA\` using \`node tools/fb-lane.cjs submit\`, records delivered work, lane-specific verification, and unresolved gates separately, and leaves a passive closeout note.
4. **Merge**: \`FB-Product\` runs verification/release gates, verifies required evidence for every lane, merges the branch to main using \`node tools/fb-lane.cjs merge\`, and releases locks.
`;
