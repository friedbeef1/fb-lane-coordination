...
*   **FB-Product (PM / Integration User Value)**: Owns final product decisions, task prioritization, scoping, file merges, staging/live deployments, and release gates. Product writes one canonical \`Working Goal\` per non-trivial task in \`PROJECT_BOARD.md\` and records goal drift as \`Goal changed from X to Y because Z.\` when it changes.
...
*   **Lightweight Goal Alignment**: For non-trivial tasks, add a \`Goal Alignment\` section in the board detail block with \`Working Goal\`, \`Success Measure\`, and \`Gate / Review Point\` where practical. Good goal: \`Working Goal: Let a signed-in user reach the camera preview, capture one mirrored photo, and save it locally without a full-page reload.\` Bad goal: \`Working Goal: finish the feature.\` Skip this extra ceremony for micro quick tasks.

### 2. The Board Loop & Resource Locking
1. **Claim**: A thread claims or creates an item on the board, and for non-trivial tasks Product sets one canonical \`Working Goal\`, \`Success Measure\`, and \`Gate / Review Point\` before the work starts. Micro quick tasks can skip this extra ceremony.
2. **Execute**: The thread works in an isolated branch (\`tech/[feature]\` or \`design/[feature]\`).
3. **Audit**: When complete, the thread pushes the branch, moves the board item to \`Staging QA\` using \`node tools/fb-lane.cjs submit\`, records delivered work, lane-specific verification, unresolved gates, plus compact handoff fields for \`Goal Alignment\`, \`Goal Challenge / Caveat\`, and \`Evidence Against Goal\`, and leaves a passive closeout note.
...
### Lightweight Goal Alignment
- For non-trivial tasks, FB-Product owns one canonical \`Working Goal\` in \`PROJECT_BOARD.md\`.
- Add \`Goal Alignment\` to the board detail block with \`Working Goal\`, \`Success Measure\`, and \`Gate / Review Point\` where practical.
- Lane handoffs use: \`Goal Alignment\`, \`Goal Challenge / Caveat\`, and \`Evidence Against Goal\`.
